...
class IPCClient:
    ...
    async def _listen(self) -> None:
        ...
        while self._is_connected and self._websocket and not self._websocket.closed:
            try:
            ...
            if msg.type in [aiohttp.WSMsgType.CLOSE, aiohttp.WSMsgType.CLOSED, aiohttp.WSMsgType.ERROR]:
                ...
                self._logger.info(f"Connection closed/error. Will reconnect...")
                break
            else:
                try:
        ...
        # If we exit the loop and disconnected, schedule reconnect
        if not self._is_connected and not self._is_connecting:
            self._bot.loop.create_task(self._reconnect_loop())
    ...
    async def _reconnect_loop(self):
        ...
        while not self._is_connected:
            self._logger.info(f"Attempting to reconnect in {reconnect_delay}s...")
    ...
    async def connect(self):
        async with self._connect_lock:
            try:
                ...
                self._logger.error("Connection failed - cannot reach dashboard.")
                
            except aiohttp.WSServerHandshakeError as e:
                self._logger.error("Access forbidden: Missing bot ID, version mismatch, or invalid password.")
                
            except Exception as e:
                self._logger.error(f"Error occurred while connecting to dashboard: {e}")
