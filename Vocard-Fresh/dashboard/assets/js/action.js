...
function buildSettingPageHtml(guildId, data) {
    const generateField = (key, fieldData) => {
        ...
        switch (fieldData.inputType) {
            case "text":
                sectionHtml = `<input data-id="${key}" class="text-input" type="text" placeholder="${fieldData?.placeholder}" value="${fieldData.default || ""}" maxlength="${fieldData.maxLength
                    }">`
                break
...
$(document).ready(function () {
    ...
    // Expose player globally for full player access
    window.player = player

    // Persistent player update every 5 seconds for live progress
    setInterval(() => {
        if (player.currentTrack && !player.isPaused) {
            // Increment position by 5000ms (5 seconds)
            player.position = (player.position || 0) + 5000
            if (player.position > (player.currentTrack.length || 0)) {
                player.position = player.currentTrack.length
            }
            // Update the position bar
            player.updatePositionBar(player.position)
            // Also update full player if open
            if (typeof updateFullPlayerProgress === 'function') {
    ...
    // Tools menu button click
    $("#menu-tools-page").on("click", function () {
        changePage("tools-page", true)

        // Populate guild selectors from mutualGuilds
        const populateGuildSelectors = () => {
            const $announceGuildSelect = $("#announce-guild-select")
            const $auditGuildSelect = $("#audit-guild-select")
            const $welcomerGuildSelect = $("#welcomer-guild-select")

            $announceGuildSelect.find("option:not(:first)").remove()
            $auditGuildSelect.find("option:not(:first)").remove()
            $welcomerGuildSelect.find("option:not(:first)").remove()

            if (player.mutualGuilds) {
                Object.entries(player.mutualGuilds).forEach(([guildId, guild]) => {
                    const option = `<option value="${guildId}">${guild.name}</option>`
                    $announceGuildSelect.append(option)
                    $auditGuildSelect.append(option)
                    $welcomerGuildSelect.append(option)
                })
            }
        }

        // If mutualGuilds not loaded yet, request it
        if (!player.mutualGuilds) {
            player.send({ op: "getMutualGuilds" })
            // Will be populated when getMutualGuilds response comes in
            // Set a brief timeout to wait for response
            setTimeout(populateGuildSelectors, 1000)
        } else {
    ...
    // Announce Guild selection
    $("#announce-guild-select").on("change", function () {
        const guildId = $(this).val()
        const $channelSelect = $("#announce-channel-select")
        const $sendBtn = $("#send-announce-btn")
        ...
        if (guildId) {
            player.send({
                op: "getGuildChannels",
                guildId: guildId,
                callback: "announce-channel-select"
            })

            // Fetch announce role settings (if admin)
            player.send({
    ...
    // Audit Guild selection
    $("#audit-guild-select").on("change", function () {
        const guildId = $(this).val()
        const $channelSelect = $("#audit-channel-select")
        const $enableCb = $("#audit-enable")
        const $auditList = $("#audit-list")
        const $saveBtn = $("#save-audit-btn")

        // Reset fields
        $channelSelect.find("option:not(:first)").remove()
        $channelSelect.prop("disabled", true)
        $enableCb.prop("checked", false).prop("disabled", true)
        $auditList.empty()
    ...
    // Announce Channel selection & validation
    $("#announce-channel-select, #announce-title, #announce-description").on("change input", function () {
        const hasChannel = $("#announce-channel-select").val() !== ""
        const hasTitle = $("#announce-title").val().trim() !== ""
        const hasDesc = $("#announce-description").val().trim() !== ""

        $("#send-announce-btn").prop("disabled", !(hasChannel && hasTitle && hasDesc))
    })

    // Send announcement button
    $("#send-announce-btn").on("click", function () {
        const $btn = $(this)
        ...
        player.send({
            op: "sendAnnounce",
            guildId: $("#announce-guild-select").val(),
            channelId: $("#announce-channel-select").val(),
            title: $("#announce-title").val(),
            description: $("#announce-description").val(),
            color: $("#announce-color").val(),
            imageUrl: $("#announce-image").val(),
            anonymous: $("#announce-anonymous").is(":checked")
        })

        // Reset after sending
        setTimeout(() => {
            $btn.html('<span class="material-symbols-outlined">send</span> Send Announcement')
            $("#announce-title").val("")
            $("#announce-description").val("")
            $("#announce-image").val("")
            $("#announce-anonymous").prop("checked", false)
        }, 2000)
    })

    // Add Announce Role
    $("#add-announce-role-btn").on("click", function () {
        const guildId = $("#announce-guild-select").val()
        const roleId = $("#announce-role-select").val()

        if (!guildId || !roleId) {
            player.tm.showToast("error", "Please select a server and role")
            return
        }
    ...
    // Remove Announce Role (delegated event)
    $(document).on("click", ".remove-announce-role", function () {
        const guildId = $("#announce-guild-select").val()
    ...
    $("#save-audit-btn").on("click", function () {
        const guildId = $("#audit-guild-select").val()
        if (!guildId) return
        ...
        player.send({
            ...
            type: "toggle",
            guildId: guildId,
            enabled: $("#audit-enable").is(":checked"),
            channelId: $("#audit-channel-select").val()
        })
    })

    // Add Auditor
    $("#audit-add-auditor-btn").on("click", function () {
        const guildId = $("#audit-guild-select").val()
        const auditorId = $("#audit-new-auditor").val().trim()

        if (!guildId || !auditorId) return
        ...
        $("#audit-new-auditor").val("")
        // Refresh settings after short delay
        setTimeout(() => {
            player.send({ op: "getAuditSettings", guildId: guildId })
        }, 1000)
    })
    ...
    $("#force-sync-btn").on("click", function () {
        const $btn = $(this)
        $btn.prop("disabled", true).html('<span class="material-symbols-outlined">sync</span> Syncing...')

        player.send({ op: "forceSync" })

        setTimeout(() => {
            $btn.prop("disabled", false).html('<span class="material-symbols-outlined">sync</span> Force Sync Commands')
        }, 5000)
    })

    // ========== WELCOMER HANDLERS ==========

    // Welcomer Guild selection
    $("#welcomer-guild-select").on("change", function () {
        const guildId = $(this).val()
        const $welcomerChannelSelect = $("#welcomer-channel-select")
        const $goodbyeChannelSelect = $("#goodbye-channel-select")
        const $welcomerEnable = $("#welcomer-enable")
        const $goodbyeEnable = $("#goodbye-enable")
        const $saveBtn = $("#save-welcomer-btn")

        // Reset fields
        $welcomerChannelSelect.find("option:not(:first)").remove().prop("disabled", true)
        $goodbyeChannelSelect.find("option:not(:first)").remove().prop("disabled", true)
        $welcomerEnable.prop("checked", false).prop("disabled", true)
        $goodbyeEnable.prop("checked", false).prop("disabled", true)
        $("#welcomer-background").val("").prop("disabled", true)
        $("#welcomer-message").val("").prop("disabled", true)
        $("#welcomer-show-card").prop("checked", true).prop("disabled", true)
        $("#goodbye-message").val("").prop("disabled", true)
        $saveBtn.prop("disabled", true)

        if (guildId) {
            // Fetch settings (which includes textChannels)
            player.send({
                op: "getSettings",
                guildId: guildId,
                callback: "welcomer-settings"
            })
        }
    })

    // Save Welcomer Settings
    $("#save-welcomer-btn").on("click", function () {
        const guildId = $("#welcomer-guild-select").val()
        if (!guildId) return

        const $btn = $(this)
        $btn.prop("disabled", true).html('<span class="material-symbols-outlined">sync</span> Saving...')

        // Build welcomer settings
        const welcomerSettings = {
            enabled: $("#welcomer-enable").is(":checked"),
            channel_id: $("#welcomer-channel-select").val() || null,
            background_url: $("#welcomer-background").val() || null,
            message: $("#welcomer-message").val() || "Welcome {user} to **{server}**! You are member **#{count}**! 🎉",
            show_card: $("#welcomer-show-card").is(":checked")
        }

        // Build goodbye settings
        const goodbyeSettings = {
            enabled: $("#goodbye-enable").is(":checked"),
            channel_id: $("#goodbye-channel-select").val() || null,
            message: $("#goodbye-message").val() || "Goodbye **{user}**! We'll miss you. 😢"
        }
        ...
        setTimeout(() => {
            $btn.prop("disabled", false).html('<span class="material-symbols-outlined">save</span> Save Welcomer Settings')
            player.tm.showToast("success", "Welcomer settings saved!")
        }, 1500)
    })

    // ========== FULL PLAYER (SPOTIFY-LIKE) ==========

    // Open full player when clicking on miniplayer track info
    // Use ID selector for more reliability
    $(document).on("click", "#control-info", function (e) {
        // Don't open if clicking on the like button
        if ($(e.target).closest("#like-btn").length) return;

        console.log("Full player clicked! Opening...")
        openFullPlayer()
    })

    // Mobile: Open full player on touch (for phones where click may not work)
    $(document).on("touchend", "#control-info", function (e) {
        // Don't open if touching the like button
        if ($(e.target).closest("#like-btn").length) return;

        // Prevent double-trigger with click
        e.preventDefault()
        console.log("Full player touched! Opening...")
        openFullPlayer()
    })
    ...
    $("#like-btn").on("click", function (e) {
        ...
        // TODO: Send like action to server when implemented
        // For now just visual toggle
    })

    // Close full player
    $("#fp-close").on("click", function () {
        closeFullPlayer()
    })

    // Close on swipe down (mobile)
    let touchStartY = 0
    $("#fullplayer").on("touchstart", function (e) {
        touchStartY = e.originalEvent.touches[0].clientY
    })
    $("#fullplayer").on("touchend", function (e) {
    ...
    $("#fp-play").on("click", function () {
        player.send({ op: "setPaused" })
    })
    ...
    $("#fp-prev").on("click", function () {
        player.send({ op: "backTo" })
    })

    $("#fp-shuffle").on("click", function () {
        player.send({ op: "shuffle", mode: "queue" })
    })

    $("#fp-repeat").on("click", function () {
        player.send({ op: "setRepeat" })
    })

    // Full player progress bar
    $("#fp-progress-bar").on("input", function () {
        const percent = $(this).val()
        const position = Math.floor((percent / 100) * player.trackLength)
        player.send({ op: "setPosition", position: position })
    })

    // Full player volume
    $("#fp-volume-bar").on("input", function () {
        const volume = $(this).val()
        player.send({ op: "setVolume", volume: parseInt(volume) })
        updateVolumeIcon(volume)
    })

    $("#fp-volume-icon").on("click", function () {
        const $bar = $("#fp-volume-bar")
        if (parseInt($bar.val()) > 0) {
            $bar.data("prev-volume", $bar.val())
            $bar.val(0).trigger("input")
        } else {
            $bar.val($bar.data("prev-volume") || 100).trigger("input")
        }
    })

    // Lyrics toggle
    $("#fp-lyrics-toggle").on("click", function () {
        const $panel = $("#fp-lyrics-panel")
        const $btn = $(this)
        ...
        if ($panel.hasClass("active")) {
            ...
            if ($("#fp-lyrics-content").find(".fp-lyrics-lines").length === 0) {
                loadFullPlayerLyrics()
            }
        }
    })

    $("#fp-lyrics-close").on("click", function () {
        $("#fp-lyrics-panel").removeClass("active")
        $("#fp-lyrics-toggle").removeClass("active")
    })

    // Queue toggle - close full player and open queue
    $("#fp-queue-toggle").on("click", function () {
        closeFullPlayer()
        setTimeout(() => {
            if (isMobile()) {
                $(".queue-container").addClass("active")
...
function openFullPlayer() {
    ...
    // Update full player with current track info
...
function closeFullPlayer() {
    $("#fullplayer").removeClass("active")
    $("#fp-lyrics-panel").removeClass("active")
    $("#fp-lyrics-toggle").removeClass("active")
    $("body").css("overflow", "")
}

function updateFullPlayerState() {
    // Use window.player for global access
    ...
    const artworkUrl = track.artworkUrl || "/static/img/notFound.png"
    $("#fp-artwork-img").attr("src", artworkUrl)
    $("#fp-background").css("background-image", `url(${artworkUrl})`)

    // Update track info
    $("#fp-title").text(track.title || "Unknown Track")
    $("#fp-artist").text(track.author || "Unknown Artist")

    // Update play/pause button
    const playIcon = p.isPaused ? "play_arrow" : "pause"
    $("#fp-play-icon").text(playIcon)

    // Update progress
    updateFullPlayerProgress()

    // Update repeat button state
    const repeatMode = p.repeat || "off"
    const $repeatBtn = $("#fp-repeat")
    $repeatBtn.find("span").text(repeatMode === "track" ? "repeat_one" : "repeat")
    $repeatBtn.toggleClass("active", repeatMode !== "off")

    // Update shuffle button state
    $("#fp-shuffle").toggleClass("active", p.shuffle || false)

    // Update volume
    $("#fp-volume-bar").val(p.volume || 100)
    updateVolumeIcon(p.volume || 100)
}
...
function updateFullPlayerProgress() {
    ...
    const length = p.currentTrack?.length || 1
    const percent = (position / length) * 100

    $("#fp-progress-bar").val(percent)
    $("#fp-progress-bar").css("background", `linear-gradient(to right, var(--primary) ${percent}%, rgba(255,255,255,0.2) ${percent}%)`)
    $("#fp-current-time").text(msToReadableTime(position))
    $("#fp-total-time").text(msToReadableTime(length))
}

function updateVolumeIcon(volume) {
    let icon = "volume_up"
    if (volume == 0) icon = "volume_off"
    else if (volume < 50) icon = "volume_down"
    $("#fp-volume-icon").text(icon)
}

function loadFullPlayerLyrics() {
    const $content = $("#fp-lyrics-content")

    if (!player.currentTrack) {
        $content.html('<div class="fp-lyrics-notfound"><span class="material-symbols-outlined">music_off</span><p>No track playing</p></div>')
        return
    }

    // Show loading
    $content.html('<div class="fp-lyrics-loading"><div class="loader"></div><span>Loading lyrics...</span></div>')

    // Request lyrics via player
    player.send({
        op: "getLyrics",
        title: player.currentTrack.title,
        artist: player.currentTrack.author,
        callback: "fullplayer-lyrics"
    })
}

// Handle lyrics response (add to objects.js response handler)
function handleFullPlayerLyrics(data) {
    const $content = $("#fp-lyrics-content")

    if (!data.lyrics || Object.keys(data.lyrics).length === 0) {
        $content.html('<div class="fp-lyrics-notfound"><span class="material-symbols-outlined">lyrics</span><p>Lyrics not found for this track</p></div>')
        return
    }

    // Get first lyrics (usually "default")
    const firstKey = Object.keys(data.lyrics)[0]
    const lyricsLines = data.lyrics[firstKey] || []

    let html = '<div class="fp-lyrics-lines">'
    lyricsLines.forEach(line => {
        html += `<p class="line">${line.replace(/\n/g, "<br>")}</p>`
    })
    html += '</div>'
