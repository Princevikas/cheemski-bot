...
const methods = {
    ...
    initBot: function (player, data) {
        ...
        // Hide/show Tools menu based on admin status
        if (bot.isAdmin) {
            $("#menu-tools-page").show();
        } else {
            $("#menu-tools-page").hide();
        }

        // Update sidebar stats with dynamic bot data
        if (data.serverCount !== undefined) {
            $("#stat-servers").text(data.serverCount.toLocaleString());
        }
        if (data.commandCount !== undefined) {
            $("#stat-commands").text(data.commandCount);
        }
        if (data.userCount !== undefined) {
            $("#stat-users").text(data.userCount.toLocaleString());
        }
        if (data.uptime !== undefined) {
            $("#stat-uptime").text(data.uptime);
        }

        if (data.activePlayers !== undefined) {
            $("#stat-activeplayers").text(data.activePlayers);
        }

        // Update bot name and avatar in stats card
        $(".bot-stats-name").text(data.botName || "Cheemski");
        if (data.botAvatar) {
            $(".bot-stats-icon").attr("src", data.botAvatar);
        }
    },
    ...
    getMutualGuilds: function (player, data) {
        ...
        // Store mutualGuilds for use in Tools page
        player.mutualGuilds = data.mutualGuilds;
    ...
    getGuildChannels: function (player, data) {
        console.log("getGuildChannels received:", data);

        const targetId = data.callback || "announce-channel-select";
        const $channelSelect = $(`#${targetId}`);
        const $sendBtn = targetId === "announce-channel-select" ? $("#send-announce-btn") : $("#save-audit-btn");

        // Get the corresponding guild select to verify guildId matches
        const guildSelectId = targetId === "announce-channel-select" ? "announce-guild-select" : "audit-guild-select";
        const currentGuildId = String($(`#${guildSelectId}`).val());
        const responseGuildId = String(data.guildId);

        console.log(`Channel response for guild: ${responseGuildId}, current selection: ${currentGuildId}`);

        // Only populate if the response matches the currently selected guild (prevents race conditions)
        if (responseGuildId && currentGuildId && responseGuildId !== currentGuildId) {
            console.log(`Ignoring stale channel response for guild ${responseGuildId}, current is ${currentGuildId}`);
            return;
        }
        ...
        if (data.channels && data.channels.length > 0) {
            console.log(`Populating ${data.channels.length} channels`);
            data.channels.forEach(channel => {
                $channelSelect.append(`<option value="${channel.id}">#${channel.name}</option>`);
            });
            $channelSelect.prop("disabled", false);

            // Check if there's a pending value to select (from getAuditSettings)
            const pendingValue = $channelSelect.data("pending-value");
            if (pendingValue) {
                $channelSelect.val(pendingValue);
                $channelSelect.removeData("pending-value");
            }
        } else {
            console.log("No channels in response");
            $channelSelect.append(`<option value="" disabled>No text channels found</option>`);
        }
    },

    getAuditSettings: function (player, data) {
        $("#audit-enable").prop("checked", data.enabled).prop("disabled", false);
        $("#audit-channel-select").prop("disabled", false);

        // Store channel ID to be selected when channels are loaded
        if (data.channelId) {
            const $select = $("#audit-channel-select");
            // If options already loaded
            if ($select.find(`option[value="${data.channelId}"]`).length) {
                $select.val(data.channelId);
            } else {
                $select.data("pending-value", data.channelId);
            }
        }
    ...
    updateAuditSettings: function (player, data) {
        if (data.status === "success") {
            player.tm.showToast("success", "Audit settings updated!");
        } else {
            player.tm.showToast("error", data.msg);
        }
    },

    sendAnnounce: function (player, data) {
        $("#send-announce-btn").prop("disabled", false).html('<span class="material-symbols-outlined">send</span> Send Announcement');
    ...
    getAnnounceSettings: function (player, data) {
        console.log("getAnnounceSettings received:", data);

        // Populate available roles dropdown
        const $roleSelect = $("#announce-role-select");
        $roleSelect.empty();
        $roleSelect.append(`<option value="">Select a role to add...</option>`);

        data.availableRoles.forEach(role => {
            // Skip roles that are already added
            const alreadyAdded = data.announceRoles.some(r => r.id === role.id);
            if (!alreadyAdded) {
        ...
        const $roleList = $("#announce-roles-list");
        $roleList.empty();

        if (data.announceRoles.length === 0) {
            $roleList.append(`<div class="no-roles-text">No custom roles configured. Only moderators can use announce.</div>`);
        } else {
    ...
    updateAnnounceSettings: function (player, data) {
        if (data.status === "success") {
            player.tm.showToast("success", "Announce roles updated!");
            // Refresh settings
            player.send({
    ...
    forceSync: function (player, data) {
        if (data.status === "success") {
            player.tm.showToast("success", data.msg);
        } else {
            player.tm.showToast("error", data.msg);
    ...
    getSessions: function (player, data) {
        ...
        if (data.sessions && data.sessions.length > 0) {
                ...
                $list.append(`
                    <div class="session-item" data-id="${session.id}">
                        <div class="session-info">
                            ...
                            <span class="session-details">Guild: ${session.guild_id} • Tracks: ${session.track_count}</span>
                        </div>
            ...
            $list.append(`<p style="color: var(--text-muted); font-style: italic;">No active sessions found.</p>`);
        }
    },

    clearSessions: function (player, data) {
        if (data.status === "success") {
            player.tm.showToast("success", data.msg);
            $("#session-list").html(`<p style="color: var(--text-muted); font-style: italic;">No active sessions found.</p>`);
        } else {
            player.tm.showToast("error", data.msg);
        }
    },

    deleteSession: function (player, data) {
        if (data.status === "success") {
            player.tm.showToast("success", data.msg);
            $(`#session-list .session-item[data-id="${data.sessionId}"]`).slideUp(200, function () {
                $(this).remove();
...
class Player {
    ...
    updateInboxList() {
        ...
        this.inboxes.forEach((mail) => {
            ...
            // Handle suggestion-type messages
            else if (mail?.type === "suggestion") {
                let date = mail?.created_at ? new Date(mail.created_at) : new Date();
                let senderName = mail?.sender?.name || "Anonymous";
                let category = mail?.category || "suggestion";
                ...
                <div class="message suggestion" data-id="suggestion-${Date.now()}">
                    ...
                    <div>
                        <p class="title">💡 ${category.charAt(0).toUpperCase() + category.slice(1)} from ${senderName}</p>
                        <p class="time">${date.toDateString()} • ${date.toLocaleTimeString()}</p>
